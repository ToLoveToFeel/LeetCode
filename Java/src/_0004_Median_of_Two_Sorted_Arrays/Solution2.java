...
public class Solution2 {
    ...
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        for (int k = 0; k < n / 2 + 1; k++) {
            pre = cur;
            if (i >= nums1.length) cur = nums2[j++];  // 如果左半部分元素已经全部处理完毕
            else if (j >= nums2.length) cur = nums1[i++];  // 如果右半部分元素已经全部处理完毕
            else if (nums1[i] < nums2[j]) cur = nums1[i++];  // 左半部分所指元素 < 右半部分所指元素
            else cur = nums2[j++];  // 左半部分所指元素 >= 右半部分所指元素
        }
