...
public class Solution {
    ...
    public List<List<Integer>> levelOrder(TreeNode root) {
        ...
        LinkedList<MyPair> q = new LinkedList<>();  // q作为队列
        q.addLast(new MyPair(root, 0));
        while (!q.isEmpty()) {
            MyPair front = q.removeFirst();
            TreeNode node = front.node;
            Integer level = front.val;
            ...
            if (null != node.left)
                q.addLast(new MyPair(node.left, level + 1));
            if (null != node.right)
                q.addLast(new MyPair(node.right, level + 1));
        }
