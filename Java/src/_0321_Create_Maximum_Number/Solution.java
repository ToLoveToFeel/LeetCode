...
public class Solution {
    ...
    // 返回保留 len 位数据的最大值
    private int[] pickKMaxNumber(int[] nums, int len) {
        ...
        int[] res = new int[len];
        int length = stack.size() - len;  // 多余的数据个数
        for (int i = 0; i < length; i++) {  // 出栈多余的数据
            stack.pop();
        }
        for (int i = res.length - 1; i >= 0; i--) {  // 得到结果
            res[i] = stack.pop();
        }
